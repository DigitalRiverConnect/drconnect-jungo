...
namespace N2.Persistence.MongoDB
{
    ...
    public class MongoContentItemRepository : MongoDbRepository<ContentItem>, IContentItemRepository
    {
        ...
        {
            const string map = "function() { emit(this._t, 1); }";
            const string reduce = "function(key, emits) { var total = 0; for (var i in emits) { total += emits[i]; }; return total; }";
        ...
        {
            return Collection.Find(Query.EQ("_t", typeof(T).Name)).Cast<T>();
        }
    }
}
