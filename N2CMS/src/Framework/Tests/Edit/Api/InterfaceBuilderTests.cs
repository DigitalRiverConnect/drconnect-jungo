...
namespace N2.Tests.Edit.Api
{
	...
	public class InterfaceBuilderTests : DatabasePreparingBase
	{
		...
		{
			var definition = builder.GetInterfaceDefinition(new FakeHttpContext(), new SelectionUtility(item, null));
		    var names = definition.MainMenu.Children.Select(m => m.Current.Name).ToArray();
                
            names.Contains(expectedMenuItem).ShouldBe(true);
		}
		...
		{
            // TODO var currentUser = engine.RequestContext.User;
            // currentUser.Identity.Name.ShouldNotBe("");

            var currentUser = new GenericPrincipal(new GenericIdentity("test"), new string[0]); 
