...
namespace N2
{
	...
	public static class Utility
	{
		...
		public static object GetProperty(object instance, string propertyName, bool notFoundIsNull = false)
		{
			if (instance == null) throw new ArgumentNullException("instance");
			if (propertyName == null) throw new ArgumentNullException("propertyName");
		...
		public static IEnumerable<ContentItem> UpdateSortOrder(IEnumerable<ContentItem> siblings)
		{
			...
			foreach (ContentItem current in siblings)
			{
				if (current.SortOrder <= lastSortOrder)
				{
					...
					if (last != null && gapBeforeLast > 1 && gapBeforeCurrent > 2)
					{
						if (index == 1)
						{
		...
		{
		    return source == destination || Find.EnumerateParents(destination).Any(ancestor => ancestor == source);
		}
		...
		public static bool IsPublished(this ContentItem item)
		{
			...
			switch (item.State)
			{
				...
				case ContentState.None:
                    return true; // TODO - review use of these for published items
        ...
        public static bool IsFuturePublished(this ContentItem item)
        {
            if (item.FuturePublishDate.HasValue && item.FuturePublishDate.Value > Utility.CurrentTime())
                return true;
        ...
        // TODO: only used by EditableNumberAttribute
