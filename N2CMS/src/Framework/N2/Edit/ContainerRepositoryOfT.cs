...
namespace N2.Edit
{
	...
	public class ContainerRepository<T> where T: ContentItem
	{
	    ...
	    public virtual T Get(ContentItem containerContainer, string name = null) 
		{
            ...
            if (result != null) cache[key] = result.ID;
		...
		{
			var result = Get(containerContainer, name);
            if (result == null)
            {
                result = Create(containerContainer, setupCreatedItem, name);
                if (result != null) cache[name ?? "<null>"] = result.ID;
            }
		    return result;
		}
