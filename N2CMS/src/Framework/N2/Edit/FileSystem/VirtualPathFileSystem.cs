...
namespace N2.Edit.FileSystem
{
	...
	public class VirtualPathFileSystem : IFileSystem
	{
		...
		public virtual DirectoryData GetDirectory(string virtualPath)
		{
			...
			d.Name = dir.Name;
            d.VirtualPath = dir.VirtualPath.TrimEnd('/');
		...
		{
			string dirpath = MapPath(virtualPath);
		...
		public virtual void WriteFile(string virtualPath, Stream inputStream, DateTime? lastWriteTime = null)
		{
			string path = MapPath(virtualPath);
            ...
            if (lastWriteTime.HasValue)
                File.SetLastWriteTimeUtc(path, lastWriteTime.Value.ToUniversalTime());
		...
		{
			using (Stream sourceFile = OpenFile(virtualPath, true))
			{
				byte[] buffer = new byte[32768];
				while (true)
				{
