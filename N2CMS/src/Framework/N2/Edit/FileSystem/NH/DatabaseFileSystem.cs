...
namespace N2.Edit.FileSystem.NH
{
	...
	public class DatabaseFileSystem : IFileSystem
	{
        ...
        public void WriteFile(string virtualPath, Stream inputStream, DateTime? lastWriteTime = null)
        {
            ...
            {
                CreateFile(FileSystemPath.File(virtualPath), inputStream, lastWriteTime);
            }
            else
            {
                UpdateFile(FileSystemPath.File(virtualPath), inputStream, lastWriteTime);
            }
	    ...
	    public void SetLastWriteTime(string virtualPath, DateTime lastWriteTime)
	    {
        ...
        internal void UpdateFile(FileSystemPath virtualPath, Stream inputStream, DateTime? lastWriteTime = null)
        {
			...
			file.Updated = lastWriteTime ?? Utility.CurrentTime();
        ...
        private void CreateFile(FileSystemPath virtualPath, Stream inputStream, DateTime? lastWriteTime = null)
        {
			...
			using (var trx = Session.BeginTransaction())
			{
				...
				{
					Path = virtualPath,
					Created = lastWriteTime ?? Utility.CurrentTime(),
					Updated = lastWriteTime ?? Utility.CurrentTime(),
					Length = fileSize
				};
				Session.Save(item);
