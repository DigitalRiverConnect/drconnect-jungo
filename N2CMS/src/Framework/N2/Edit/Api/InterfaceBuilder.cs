...
namespace N2.Management.Api
{
    ...
    public class InterfaceMenuItem
    {
        ...
        {
            get { return _controller ?? (_controller = "PageActionCtrl"); }
    ...
    public class InterfaceBuilder
    {
        ...
        protected virtual Node<InterfaceMenuItem> GetNode(LinkPluginAttribute np)
        {
            ...
            if (np is ToolbarPluginAttribute)
            {
                ...
                {
                    var options = (IProvider<ToolbarOption>)engine.Container.Resolve(tp.OptionProvider);
                    node.Children = options.GetAll().Select(GetNode).ToList();
                }

                if (!string.IsNullOrEmpty(tp.CustomController))
                    node.Current.Controller = tp.CustomController;

                if (!string.IsNullOrEmpty(tp.ClientAction))
                    node.Current.ClientAction = tp.ClientAction;
            }
            return node;
        }
