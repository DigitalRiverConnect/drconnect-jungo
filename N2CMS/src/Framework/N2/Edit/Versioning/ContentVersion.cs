...
namespace N2.Edit.Versioning
{
	public sealed class ContentVersion
	{
		...
		public static void ReorderBySortOrderRecursive(ContentItem item)
		{
			if (item.Children.Count > 0)
			{
                if (item.Children.Count > 1) // TODO check list type
				    item.Children = new Collections.ItemList(item.Children.OrderBy(i => i.SortOrder).ThenBy(i => i.ID));
