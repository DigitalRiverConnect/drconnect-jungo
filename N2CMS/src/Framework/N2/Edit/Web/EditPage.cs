...
namespace N2.Edit.Web
{
    ...
    public class EditPage : Page, IProvider<IEngine>
	{
		...
		{
			if (IsPostBack && Selection.SelectedItem == null && !string.IsNullOrEmpty(Request["selected"]))
		...
		{
		    if (item == null) return; // should be ok to allow access to a null item ;-) this can happen to start page after initial setup
		...
		{
			return (selectedItem != null) ? Page.GetNavigationUrl(Engine, selectedItem) : "";
		}

		protected virtual string GetPreviewUrl(ContentItem selectedItem)
		{
			return (selectedItem != null) ? Page.GetPreviewUrl(Engine, selectedItem) : "";
		}
		#endregion
