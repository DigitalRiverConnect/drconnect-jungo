...
namespace N2.Engine
{
	...
	{
		private readonly Logger<ContentAdapterProvider> logger;
        ...
        /// <param name="adaptersToRemove">The adapters to remove.</param>
		...
		private AbstractContentAdapter CreateAdapter(Type adapterType, Type contentType)
		{
			...
			if (!ContainsServiceOfType(adapterType))
			{
		...
		private bool ContainsServiceOfType(Type adapterType)
		{
		    var services = engine.Container.ResolveAll(adapterType);
		    return services.Any(service => service.GetType() == adapterType);
		}
