...
namespace N2.Plugin.Scheduling
{
    ...
    public class Scheduler : IAutoStart
    {
		...
		public Scheduler(IEngine engine, IHeart heart, IWorker worker, IWebContext context, IErrorNotifier errorHandler, IEnumerable<ScheduledAction> registeredActions, Configuration.EngineSection config)
        {
			this.engine = engine;
			this.heart = heart;
			this.worker = worker;
