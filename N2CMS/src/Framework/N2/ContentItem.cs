...
namespace N2
{
    ...
    /// </remarks>
	[Serializable, DebuggerDisplay("{TypeName, nq} #{ID} {Path, nq} {State}")]
	[DynamicTemplate]
			...
			get 
			{
				if(url == null || url.Contains("?"))
				{
					if (urlParser != null)
                        ...
                        url = N2.Context.Current.UrlParser.BuildUrl(this); // use ambient context instead of property DI (speed)
//						url = FindPath(PathData.DefaultAction).GetRewrittenUrl();
		...
		public virtual T GetDetail<T>(string detailName, T defaultValue)
		{
			...
			try
			{
			    ...
			    return (T)(o ?? default(T)); // Attempt regular cast conversion
			}
		...
		{
		    var ret = DetailCollections[collectionName];
			if (ret != null)
				return ret;
		...
		static void CloneDetails(ContentItem source, ContentItem destination)
		{
			foreach (ContentDetail detail in source.Details.Values)
			{
			    ...
			    var d = destination.details[detail.Name];
				if(d != null) 
				{
                    d.Value = clonedDetail.Value;//.Value should behave polymorphically
				} 
				else 
				{
		...
		{
		    return ID == 0 ? string.Format("{0} {{{1} >{2}}}", GetContentType().Name, Path, State) 
                           : string.Format("{0} {{{1} #{2}}}", GetContentType().Name, Name, ID);
		}
		...
		private void ClearMissingDetails(ContentItem source, ContentItem destination)
		{
			...
			{
			    var s = source.DetailCollections[key];
				if (s != null)
				{
        ...
        /// <summary>
        /// Property to avoid use of Magic String 
        /// TODO: consider storing as column, not detail
        /// </summary>
        [NonInterceptable]
        public DateTime? FuturePublishDate
        {
            get { return GetDetail("FuturePublishDate") as DateTime?; }
