...
namespace N2.Web.Rendering
{
	...
	public class DisplayableRendererSelector
	{
		...
		{
		    if (context.IsEditable) // why else would we call this?
		    {
		        var tw = WebExtensions.GetEditableWrapper(context.Content, context.IsEditable, context.PropertyName,
		                                                  context.Displayable, writer);
		        using (tw)
		        {
