...
namespace N2.Web
{
	public class CachingUrlParserDecorator : IUrlParser, IFlushable
	{
	    public static string _n2Pathdatacache = "N2.PathDataCache";
        ...
        public static readonly HashSet<string> allCacheKeys = new HashSet<string>(); 
	    ...
	    private Dictionary<string, PathData> GetCachedPathData()
	    {
	        Dictionary<string, PathData> cachedPathData;
	        if ((cachedPathData = cache.Get<Dictionary<string, PathData>>(_n2Pathdatacache)) == null)
	        {
	            lock (pathLock)
	            {
	                if ((cachedPathData = cache.Get<Dictionary<string, PathData>>(_n2Pathdatacache)) == null)
	                {
	                    ...
	                    cache.Add(_n2Pathdatacache, cachedPathData, new CacheOptions {SlidingExpiration = SlidingExpiration});
	                }
	            }
	        }
        ...
        public void Flush()
        {
            ...
            lock (urlLock)
            {
                try
                {
                    foreach (string cacheKey in allCacheKeys)
                        cache.Remove(cacheKey);

                    cache.Remove(_n2Pathdatacache);
