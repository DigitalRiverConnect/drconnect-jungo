...
namespace N2.Edit.Tests.FileSystem
{
	...
	[TestFixture]
	public class FileSystemTests_DatabaseFileSystem : FileSystemTests
	{
    ...
    [TestFixture]
    public class FileSystemTests_AzureStorageFileSystem : FileSystemTests
    {
        protected override IFileSystem CreateFileSystem()
        {
            var connectionStrings = (ConnectionStringsSection)ConfigurationManager.GetSection("connectionStrings");
            return new Azure.AzureStorageFileSystem(connectionStrings);
        }
    }
    ...
    public abstract class FileSystemTests : DatabasePreparingBase
	{
        ...
        public void GetFiles_RetrievesFiles_BelowRoot()
        {
            ...
            Assert.That(fs.FileExists("/upload/hello.txt"));
		...
		{
            Assert.That(!fs.DirectoryExists("/upload/hello"));
            Assert.That(!fs.DirectoryExists("/upload"));
			fs.CreateDirectory("/upload/hello");
			var d = fs.GetDirectory("/upload");

			Assert.That(d, Is.Not.Null);
			Assert.That(d.Name, Is.EqualTo("upload"));
            Assert.That(d.VirtualPath, Is.EqualTo("/upload"));
            //Assert.That(d.Created.Kind.Equals(DateTimeKind.Utc));
		    var age = (DateTime.UtcNow - d.Created.ToUniversalTime()).TotalMinutes;
            Assert.That(age < 1); // new date

            // ensure slash is trimmed and VirtualPath is not just a pass-thru
            d = fs.GetDirectory("/upload/"); 
            Assert.That(d, Is.Not.Null);
            Assert.That(d.Name, Is.EqualTo("upload"));
            Assert.That(d.VirtualPath, Is.EqualTo("/upload"));
        }
		...
		public void OpenFile_ReadsFileSizes(string text)
		{
			...
			using (var s = fs.OpenFile("/upload/hello/world.txt", true))
		...
		public void WriteFile_ModifiesFileContents()
		{
            ...
            // MAB this fails with a "Stream Not Readable" error:
            //   var s = new StreamReader(fs.OpenFile("/upload/hello/world.txt")
            // by default OpenFile opens files in write mode-- in which case the returned
            // stream can be written to but not read from.  I'm modifying this, to pass
            // true to OpenFile.  This opens the file in read mode (which is what we want for
            // this verification test anyway).
			using (var s = new StreamReader(fs.OpenFile("/upload/hello/world.txt", true)))
			{
        ...
        [Test]
        public void WriteFileWithDate_UpdatesDate()
        {
            DateTime lastUpdated = new DateTime(2000,1,1);
            fs.WriteFile("/upload/hello.txt", new MemoryStream(Encoding.UTF8.GetBytes("hello world")), lastUpdated);

            var files = fs.GetFiles("/upload/");

            Assert.That(fs.FileExists("/upload/hello.txt"));
            Assert.That(files.Single().Name, Is.EqualTo("hello.txt"));
            Assert.That(files.Single().Updated, Is.EqualTo(lastUpdated));
        }
		...
		public void WriteFile_ExpandFileContents()
		{
			...
			// MAB AzureFileSystem::CreateFile checks for a file's existence and throws an exception
            // if it's already there.  Modifying CreateFile to delete the file if it already exists.
            fs.WriteFile("/upload/hello/world.txt", new MemoryStream(Encoding.UTF8.GetBytes("hello world")));
