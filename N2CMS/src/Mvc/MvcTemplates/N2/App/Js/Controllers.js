try {
    ...
    })(angular.module('n2', ['n2.directives', 'n2.services', 'n2.localization', 'ui', '$strap.directives', 'n2-custom-toolbar']));
} catch (err) {
    (function (app) {
        app.value('$strapConfig', {
            datepicker: {
...
function ManagementCtrl($scope, $window, $timeout, $interpolate, Context, Content, Profile, Security, FrameContext, Translate, Eventually) {
    ...
    $scope.select = function (nodeOrPath, versionIndex, keepFlags, forceContextRefresh, preventReload, disregardNodeUrl) {
        ...
        if (typeof nodeOrPath == "string") {
			var path = nodeOrPath;
            node = findNodeRecursive($scope.Context.Content, path);
            if (!node) {
    ...
    $scope.reloadNode = function (pathOrNode, callback) {
	    ...
	    Content.reload(node, function (anode) {
	        callback && callback(anode);
	    });
	};
	...
	$scope.$on("preiewloaded", function (scope, e) {
		if ($scope.Context.AppliesTo == (e.path + e.query)) {
			window.console && console.log("bailing out", $scope.Context.AppliesTo, "==", (e.path + e.query));
			//return;
		}
...
function MenuCtrl($rootScope, $scope, Security) {
	$scope.$watch("Context.ActionMenu.Children", function (children) {
		...
		for (var i in children) {
		    try {
		        children[i].Current.Controller = eval(children[i].Current.Controller);
		    } catch (err) {
		        children[i].Current.Controller = children[i].Current.Controller;
		    }
