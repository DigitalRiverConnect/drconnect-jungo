...
namespace N2.Management.Resources
{
	...
	public class IconHandler : IHttpHandler, IAutoStart
	{
		...
		public IconHandler(EventBroker broker)
		{
			this.broker = broker;
		}
		...
		private void HttpApplication_PostResolveRequestCache(object sender, EventArgs e)
		{
			...
			try
			{
			    ...
			    if (p.EndsWith(".png", StringComparison.InvariantCultureIgnoreCase))
			    {
			        const string iconBase = "resources/icons/";
			        ...
			        const string flagsBase = "resources/img/flags/";
                    if (p.IndexOf(flagsBase, StringComparison.OrdinalIgnoreCase) >= 0)
			        {
			            collection = "flags";
                        p = EatTo(p, flagsBase);
			            key = p.Substring(0, p.IndexOf(".png", StringComparison.OrdinalIgnoreCase));
