...
namespace N2.Management.Content.Versions
{
	public partial class RecentVersions : EditUserControl
	{
		...
		protected override void  OnDataBinding(EventArgs e)
		{
			...
			Versions = allVersions.Select(v => new VersionInfo { ID = v.ID, VersionIndex = v.VersionIndex, Title = v.Title, SavedBy = v.SavedBy, Info = GetVersionInfo(v), InProgress = false, State = v.State }).Take(3);
