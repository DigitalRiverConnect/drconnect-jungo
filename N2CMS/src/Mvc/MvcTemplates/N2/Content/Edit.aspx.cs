	...
	public partial class Edit : EditPage, IItemEditor
	{
		...
		protected void OnPreviewCommand(object sender, CommandEventArgs e)
		{
			...
			Url previewUrl = Engine.GetContentAdapter<NodeAdapter>(page)
				.GetPreviewUrl(page);
			if (Request["edit"] == "drag" || CreatingNew) // SW continue in organize parts after creation
		...
		protected void OnUnpublishCommand(object sender, CommandEventArgs e)
		{
            ...
            VersioningExtensions.Unpublish(Engine.Persister, item);
		...
		protected void OnSaveFuturePublishCommand(object sender, CommandEventArgs e)
		{
            ...
            Validate(); // TODO Command does validation, too
            if (IsValid && dpFuturePublishDate.SelectedDate.HasValue && dpFuturePublishDate.SelectedDate > Utility.CurrentTime())
			{
                ...
                // TODO move to state changer command
                Engine.Resolve<PublishScheduledAction>().MarkForFuturePublishing(ctx.Content, dpFuturePublishDate.SelectedDate.Value);
                Engine.Persister.Save(ctx.Content);
			}
            else
            {
                ctx.Errors.Add(new Exception("invalid publish date: " + dpFuturePublishDate.SelectedDate));
            }
            HandleResult(ctx, "");
        }
		...
		private void InitItemEditor()
		{
            ...
            dpFuturePublishDate.SelectedDate = ie.CurrentItem.FuturePublishDate ?? DateTime.Today.AddDays(1);
		}
